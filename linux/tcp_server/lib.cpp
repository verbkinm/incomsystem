...
void general::clientThread(Client *client, int socket)
{
    client = new Client(socket);
    general::thread_cout(general::currentDateTime() + ' ' + general::hostInfo(socket) + " - connected\n");
    client->exec();
    general::thread_cout(general::currentDateTime() + ' ' + general::hostInfo(socket) + " - disconnected\n");
    delete client;
    client = nullptr;
}
